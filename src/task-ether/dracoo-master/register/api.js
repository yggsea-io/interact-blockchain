...
const FAIL_PATH = path.join(__dirname, './resgister-fail.txt')
...
class Api{
    ...
    async login() {
        ...
        const signatureRes = await dracooMasterSign(this.address, signatureReg.data.message);
        let req = {
      ...
      async registerGame(name, mail) {
        ...
        if(code == false || code == undefined){
          AppendDataToFile(FAIL_PATH, `${this.address},${this.mail},Error: Cannot get verify code.Please get token to read mail again\n`)
      ...
      async setName() {
        try {
          ...
          if (data.code != "OK") {
            AppendDataToFile(FAIL_PATH, `${this.address},${this.mail},Error set name: ${this.name}\n`)
            return false;
          }
          return true;
        } catch (error) {
          console.log(
            AppendDataToFile(FAIL_PATH, `${this.address},${this.mail},Error set name: ${this.name}: ${error}\n`)
          );
          return false;
        }
      }
      ...
      async checkMail() {
        try {
          ...
          if (data.code != 200) {
            AppendDataToFile(FAIL_PATH, `${this.address},${this.mail},Error: Check mail be wrong \n`)
            return false;
          } else if (data.code == 200 && data.msg == "existence") {
            AppendDataToFile(FAIL_PATH, `${this.address},${this.mail},Error: Mail is existence \n`)
            return false;
          }
          return true;
        } catch (error) {
            AppendDataToFile(FAIL_PATH, `${this.address},${this.mail},Error: Check mail be wrong: ${error} \n`)
          return false;
        }
      }
      ...
      async  sendMail() {
        try {
          ...
          if (data.code != "OK") {
            AppendDataToFile(FAIL_PATH, `${this.address},${this.mail},Error: Cannot send mail to get code \n`)
            return false;
          }
          return true;
        } catch (error) {
      ...
      async  getVerifyCode() {
        try {
          ...
          const mess = messArr.find((mess) => mess.payload.headers.find((header) => (header.value == this.mail && header.name == 'To')));
      ...
      async  verifiMail(code) {
        try {
          ...
          if(data.code == "OK" && data.data == true){
            ...
            AppendDataToFile(FAIL_PATH, `${this.address},${this.mail},Error: Mail is registed \n`)
          } else{
            AppendDataToFile(FAIL_PATH, `${this.address},${this.mail},Error: Cannot verify mail, maybe error code \n`)
            return false;
          }
        } catch (error) {
          AppendDataToFile(FAIL_PATH, `${this.address},${this.mail},Error: Cannot verify mail : ${error} \n`)
          return false;
        }
      }
      ...
      async bindMail() {
        ...
        try {
          ...
          if (data.code != "OK") {
            AppendDataToFile(FAIL_PATH, `${this.address},${this.mail},${pass},Error: Cannot bind Mail \n`)
            return false;
          }else if(data.code == "OK"){
            AppendDataToFile(SUCCESS_PATH, `${this.address},${this.mail},${pass} \n`)
          }
          return true;
        } catch (error) {
          AppendDataToFile(FAIL_PATH, `${this.address},${this.mail},${pass},Error: Cannot bind Mail : ${error}\n`)
          return false;
        }
      }
}
